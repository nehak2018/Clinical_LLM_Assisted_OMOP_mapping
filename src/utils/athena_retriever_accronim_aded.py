...
class AthenaRetriever:
    ...
    def retrieve_keyword(
        ...
        #Acronym
        acronym = "".join(token[0] for token in query_tokens if token)
        expanded_tokens = set(query_tokens + ([acronym] if len(acronym) > 1 else []))

        vocab_mask = self.concept_df["vocabulary_id"].isin(vocabularies)
        domain_mask = self.concept_df["domain_id"] == "Condition"

        #vocab_mask = self.concept_df["vocabulary_id"].isin(vocabularies)
        #pool = self.concept_df[vocab_mask].copy()
        
        pool = self.concept_df[vocab_mask & domain_mask].copy()
        cols = ["concept_id", "concept_name", "concept_code","vocabulary_id", "standard_concept"]
